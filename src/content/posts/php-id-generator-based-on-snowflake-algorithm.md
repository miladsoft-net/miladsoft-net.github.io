...
```shell
$ composer require miladsoft/php-snowflake -vvv
```
...
```php
$snowflake = new \Miladsoft\Snowflake\Snowflake;
...
```php
$snowflake = new \Miladsoft\Snowflake\Snowflake($datacenterId, $workerId);
...
use Miladsoft\Snowflake\Snowflake;
use Miladsoft\Snowflake\LaravelSequenceResolver;
...
你可以通过实现 Miladsoft\Snowflake\SequenceResolver 接口来自定义序列号解决器。
...
如果您在使用过程中遇到任何问题，欢迎提交 「[PR](https://github.com/miladsoft/php-snowflake)」。
