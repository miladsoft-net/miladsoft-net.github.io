...
```bash
acme.sh  --issue  --dns  -d miladsoft.net -d '*.miladsoft.net' --yes-I-know-dns-manual-mode-enough-go-ahead-please
```
...
![file](https://images.miladsoft.net/posts/201806/21/1_1529590937_4rCg5hX0qu.png)
...
![file](https://images.miladsoft.net/posts/201806/21/1_1529591195_ryE5GiaXNP.png)
...
```bash
.acme.sh/acme.sh --issue --dns dns_dp -d miladsoft.net -d *.miladsoft.net
```
...
```bash
cp ~/.acme.sh/miladsoft.net/fullchain.cer /etc/nginx/ssl/fullchain.cer
cp ~/.acme.sh/miladsoft.net/miladsoft.net.key /etc/nginx/ssl/miladsoft.key
```

2、新建 `ssl-params.conf` 并把它放到 Nginx 的 `snippets` 目录中。

> 下面的这些配置来自 [提高安全性的最佳 Nginx 配置](https://miladsoft.net/posts/best-nginx-configuration-for-improved-security)，建议参考。
...
ssl_certificate_key      /etc/nginx/ssl/miladsoft.key;
...
完整的 `Nginx` 配置文件请参考 [我的 Nginx 配置](https://github.com/miladsoft/gblog/blob/master/resources/nginx/)

4、配置虚拟主机

```vim
# /etc/nginx/sites-available/miladsoft.net

server {
    listen 80 default_server;
    listen [::]:80 default_server;

    server_name miladsoft.net www.miladsoft.net;
...
server {
    ...
    server_name www.miladsoft.net miladsoft.net;

    # 引入 SSL 及 PHP 配置
    include snippets/fastcgi-php.conf;
    include snippets/ssl-params.conf;

    root /home/miladsoft/websites/miladsoft.net/public;

    access_log /home/miladsoft/websites/miladsoft.net/storage/logs/nginx-access.log;
    error_log  /home/miladsoft/websites/miladsoft.net/storage/logs/nginx-error.log error;

    index index.php;

    # 当访问域名是不  miladsoft.net 强制跳转到 https://miladsoft.net
    if ($host != 'miladsoft.net' ) {
        rewrite ^/(.*)$ https://miladsoft.net/$1 permanent;
    }
}
```

再来看一个 `admin.miladsoft.net` 的配置

```vim
# /etc/nginx/sites-available/admin.miladsoft.net

server {
    listen 80;
    listen [::]:80;

    server_name admin.miladsoft.net;
    return 301 https://$server_name$request_uri;
}
server {
    ...
    root /home/miladsoft/websites/admin.miladsoft.net/public-admin;

    access_log /home/miladsoft/websites/admin.miladsoft.net/storage/logs/nginx-access.log;
    error_log  /home/miladsoft/websites/admin.miladsoft.net/storage/logs/nginx-error.log error;
    ...
    if ($host != 'admin.miladsoft.net' ) {
        rewrite ^/(.*)$ https://admin.miladsoft.net/$1 permanent;
    }
}
...
```bash
sudo ln -s /etc/nginx/sites-available/miladsoft.net /etc/nginx/sites-enabled/
sudo ln -s /etc/nginx/sites-available/admin.miladsoft.net /etc/nginx/sites-enabled/
...
以上所有配置你都可以在 [这里](https://github.com/miladsoft/gblog/blob/master/resources/nginx/) 找到。
